                ...
                <div class="tab-pane fade" id="stages" role="tabpanel">
                    ...
                    <div class="row text-white mt-4">
                        <?php if (count($teams) == 4): ?>
                            <p>✅ Kondisi terpenuhi: Ada 4 tim</p>
                            <?php
                                shuffle($teams);
                                $match1_team1 = $teams[0];
                                $match1_team2 = $teams[1];
                                $match2_team1 = $teams[2];
                                $match2_team2 = $teams[3];
                            ?>
                            <div class="row mt-4 text-dark text-center">

                                <h5 class="fw-bold">WB Round 1</h5>
                                <div class="d-flex justify-content-center gap-5">
                                    <div>
                                        <p><?= $match1_team1['name'] ?> vs <?= $match1_team2['name'] ?></p>
                                        <p><?= $match2_team1['name'] ?> vs <?= $match2_team2['name'] ?></p>
                                    </div>
                                </div>
                            </div>
                        <?php else: ?>
                            <p class="text-danger">❌ Kondisi TIDAK terpenuhi: Jumlah tim = <?= count($teams) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
